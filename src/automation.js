...
const BUSY_TIMEOUT = 5 * 60 * 1000; // busy超过5分钟视为卡死，强制重置
...
function start(config) {
  ...
  // 自愈看门狗：每2分钟检查一次，如果busy卡死超过5分钟则强制重置
  engine.watchdogTimer = setInterval(() => {
    if (engine.busy && engine.busySince > 0) {
      const stuckMs = Date.now() - engine.busySince;
      if (stuckMs > BUSY_TIMEOUT) {
        ...
        // 立即触发一次搜索
        searchAndProcess().catch(err => {
          console.error('[automation] 看门狗触发搜索失败:', err.message);
        });
      }
    }
    // 检查lastSearchTime是否过时（即使busy=false但长时间没搜索）
    if (engine.lastSearchTime > 0 && !engine.busy) {
      const sinceLastSearch = Date.now() - engine.lastSearchTime;
      if (sinceLastSearch > interval * 3) {
        console.warn(`[automation] 看门狗: 距上次搜索已${Math.round(sinceLastSearch / 1000)}秒，触发搜索`);
        logger.log('system', `看门狗触发搜索（距上次${Math.round(sinceLastSearch / 1000)}秒）`, {});
        searchAndProcess().catch(err => {
          console.error('[automation] 看门狗触发搜索失败:', err.message);
        });
      }
...
function stop() {
  ...
  if (engine.watchdogTimer) {
    clearInterval(engine.watchdogTimer);
    engine.watchdogTimer = null;
  }
...
function updateConfig(config) {
  const oldInterval = (engine.config && engine.config.guanchen && engine.config.guanchen.searchInterval) || DEFAULT_SEARCH_INTERVAL;
  const newInterval = (config.guanchen && config.guanchen.searchInterval) || DEFAULT_SEARCH_INTERVAL;
  engine.config = config;

  // 如果引擎正在运行且间隔变了，重启定时器
  if (engine.running && oldInterval !== newInterval) {
    console.log(`[automation] 搜索间隔变更: ${oldInterval / 1000}秒 → ${newInterval / 1000}秒，重启定时器`);
    if (engine.timer) {
      clearInterval(engine.timer);
    }
    engine.timer = setInterval(() => {
...
async function searchAndProcess() {
  ...
  if (!engine.config || !engine.config.guanchen) {
    ...
    logger.log('auto_search', '搜索跳过：观尘配置缺失', {});
    return;
  }
  ...
  if (!gcfg.apiKey) {
    ...
    logger.log('auto_search', '搜索跳过：API Key未配置', {});
    return;
  }
  ...
  try {
    ...
    logger.log('auto_search', `搜索失败: ${err.message}`, {
      error: err.message,
      busyDuration: engine.busySince ? Math.round((Date.now() - engine.busySince) / 1000) : 0
    });
    // 不停止引擎，等待下个周期重试
  } finally {
    engine.busy = false;
